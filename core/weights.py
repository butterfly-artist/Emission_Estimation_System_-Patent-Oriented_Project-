...
def get_top_leverage_stations(
    ...
    logger.info(
        f"Top {k} stations: {top_idx}, "
        f"R_i values: {R_i[top_idx]}"
    )
    return top_idx
...
def wind_adjusted_weights(
    ...
    center_pos = np.mean(
        station_positions, axis=0
    )
    delta = station_positions - center_pos[
        np.newaxis, :
    ]
    station_angle = np.arctan2(
        delta[:, 1], delta[:, 0]
    )
    wind_rad = np.radians(wind_direction_deg)
    alignment = np.cos(
        wind_rad - station_angle
    )
    R_i_adjusted = R_i * (
        1 + beta * alignment
    )
    ...
    if adj_max > adj_min:
        R_i_adjusted = (
            R_i_adjusted - adj_min
        ) / (adj_max - adj_min)
    else:
        R_i_adjusted = np.full_like(
            R_i_adjusted, 0.5
        )
    return R_i_adjusted
...
if __name__ == "__main__":
    ...
    top_idx = get_top_leverage_stations(
        R_i, k=5
    )
    ...
    print(
        f"All R_i in [0,1]: "
        f"{(R_i>=0).all() and (R_i<=1).all()}"
    )
    print(f"Top 5 stations: {top_idx}")
    print(f"Their zones: ", end="")
    for idx in top_idx:
        print(
            data['zone_labels'][idx], end=" "
        )
    print()
    print("weights.py verification PASSED")
